...
class RecurrentAgent(S_Agent):
    ...
    def initial_state(self, agent_info, B):
        return DictTensor(
            {
                "agent_state": self.model.action_model.initial_state(B),
    ...
    def __call__(self, state, observation, agent_info, history=None):
        ...
        new_z, action_proba = self.model.action_model(
            state["agent_state"], observation["frame"]
        )
        ...
        new_state = DictTensor(
            {"agent_state": new_z, "agent_step": state["agent_step"] + 1}
        )
